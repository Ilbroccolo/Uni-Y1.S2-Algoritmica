...
\textbf{Come Applicarlo in un Esercizio: Passo Passo}

\begin{example}[Guida Passo-Passo: $T(n) = 9 \cdot T(\frac{n}{3}) + n$]

    \textbf{Passo 1: Identificare $a$, $b$, e $f(n)$}
    ...
    \textbf{Passo 2: Calcolare il "Confine" Ricorsivo}
    Calcola il valore $n^{\log_b{a}}$.
    \begin{itemize}
        \item $n^{\log_3 9} = n^2$
    \end{itemize}

    \textbf{Passo 3: Confrontare $f(n)$ con $n^{\log_b{a}}$}
    Confrontiamo $f(n) = n$ con $n^2$.
    \textbf{I Tre Casi del Teorema}
    \textbf{Caso 1: $f(n)$ cresce più LENTAMENTE}
    \begin{itemize}
    ...
    \textbf{Caso 3: $f(n)$ cresce più VELOCEMENTE}
    \begin{itemize}
