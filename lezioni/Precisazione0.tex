...
\section{Differenza Chiave: \texorpdfstring{$O(n)$}{O(n)} (Lineare) vs. \texorpdfstring{$O(\log n)$}{O(log n)} (Logaritmico)}
...
\begin{example}[Gerarchia delle Complessità]
    \begin{description}
        \item[$O(1)$ - Costante:] Il tempo non dipende da $n$. (Es. Accesso a un array \texttt{array[i]}).
...
\begin{observation}[Regole di Calcolo Asintotico]
    \begin{itemize}
        ...
        L'esempio classico è un \texttt{for} $O(n)$ che contiene un altro \texttt{for} $O(n)$: la complessità totale è $O(n \times n) = O(n^2)$.
    \end{itemize}
\end{observation}
...
\begin{example}[Esempio 1: Ciclo Singolo (Lineare)]
    ...
    \end{lstlisting}
    \textbf{Analisi ($O(n)$):} Il codice esegue due comandi iniziali $O(1)$. Segue un ciclo \texttt{while}.
...
\begin{example}[Esempio 5: Condizionale nel Caso Pessimo]
    ...
    Il ciclo esterno (while $i<n$) viene eseguito $n$ volte. All'interno c'è un \texttt{if}.
    Dobbiamo considerare il costo del ramo più pesante.
    \begin{itemize}
        \item Il ramo \texttt{if} ($i<10$) ha costo $O(1)$.
        \item Il ramo \texttt{else} contiene un ciclo lineare, costo $O(n)$.
    \end{itemize}
    Nell'analisi del caso pessimo, assumiamo che venga sempre eseguito il ramo più costoso.
    Quasi tutte le iterazioni (per $i \ge 10$) eseguiranno il ramo \texttt{else}, che costa $O(n)$.
