...
\begin{explanation}{Ricerca del Minimo}
Algoritmo iterativo standard:
\begin{itemize}
    \item Assume che il primo elemento sia il minimo provvisorio.
    \item Scorre il resto dell'array: se trova un elemento minore, aggiorna il minimo.
    \item Costo sempre lineare $\Theta(n)$.
\end{itemize}
\end{explanation}
...
\begin{explanation}{Ricerca Lineare (con K)}
Simile alla ricerca del minimo, ma si ferma appena trova l'elemento $k$.
Il caso ottimo è $O(1)$ (trovato subito), il pessimo è $O(n)$ (in fondo o assente).
\end{explanation}

\textbf{Analisi:}
\begin{itemize}
    \item \textbf{Caso Ottimo}: $k = A[1]$. Il ciclo \texttt{while} esegue 1 iterazione. $T(n) \in \Theta(1)$ (Costante).
    \item \textbf{Caso Pessimo}: $k \notin A$ (o $k=A[n]$). Il ciclo \texttt{while} esegue $n$ iterazioni. $T(n) \in \Theta(n)$ (Lineare).
\end{itemize}
...
\begin{explanation}{Ricerca Binaria Iterativa}
Versione non ricorsiva della Binary Search.
Usa un ciclo \texttt{while} per aggiornare gli estremi $p$ e $r$.
È più efficiente in termini di spazio (niente stack ricorsivo) ma identica come complessità temporale $O(\log n)$.
\end{explanation}
